...
public class Regeneration implements Listener {
    ...
    private void regenerate(final List<Block> list, final Location location, String cause) {
        ...
        // Check if cause is whitelisted
        List<String> explosionWhitelist = plugin.getConfigManager().getExplosionWhitelist();
        if (!(explosionWhitelist.size() == 1 && explosionWhitelist.get(0).equals("*"))) {
            if (!explosionWhitelist.contains(cause)) return;
        }
                        ...
                        caches.forEach((block, cache) -> {
                            ...
                            switch (material) {
                                ...
                                case BLACK_SHULKER_BOX, BLUE_SHULKER_BOX, BROWN_SHULKER_BOX, CYAN_SHULKER_BOX,
                                     ...
                                     SHULKER_BOX, WHITE_SHULKER_BOX, YELLOW_SHULKER_BOX ->
    ...
    public void onBlockExplode(final BlockExplodeEvent event) {
        ...
        this.regenerate(event.blockList(), event.getBlock().getLocation(), event.getBlock().getType().name());
    }
    ...
    public void onEntityExplode(final EntityExplodeEvent event) {
        ...
        this.regenerate(event.blockList(), event.getLocation(), event.getEntity().getType().name());
    }
